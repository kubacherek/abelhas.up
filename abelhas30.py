...
class Chomik(object):
    ...
    def send(self, content):
        ...
        while counter < 14:
            try:
                ...
                kRespSize = 2056
                while True:
                    tmp_bytes = sock.recv(kRespSize)
                    tmp = tmp_bytes.decode('utf-8', errors='ignore')
                    resp   += tmp
                ...
                self.view.print_("Proba "+str(counter)+" z 14")
                counter += 1
    ...
    def relogin(self):
        ...
        xml_len = len(xml_content.encode('utf-8'))
    ...
    def __upload_get_tokens(self, filepath, filename):
        ...
        filename_tmp = escape_name(change_coding(filename))
        filename_len = len(filename_tmp)
        xml_dict = [('ROOT',[('token' , self.ses_id), ('folderId' , self.folder_id), ('fileName', filename_tmp) ])]
    ...
    def __create_header(self, server, port, token, stamp, filename, size, resume_from = 0):
        ...
        contentlength = len(contentheader.encode('utf-8')) + (size - 2) + len(contenttail.encode('utf-8'))
        ...
        return header.encode("utf8"), contenttail.encode("utf8")
...
def usage():
    ...
    print('-r,--recursive\t\t wysyla zawartosc katalogu (oraz wszystkich podkatalogow) na chomika do wskazanego katalogu. Na chomiku tworzona jest cala struktura podkatalogow. Przykład:', end=' ')
    print('python', sys.argv[0], '-r "/katalog1/katalog2/katalog3" "/home/nick/Dokumenty"')
    print('-u,--upload\t\t wysyla plik na chomika do wskazanego katalogu.Przykład:', end=' ')
